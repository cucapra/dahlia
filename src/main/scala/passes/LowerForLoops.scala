...
object LowerForLoops {
  ...
  private final case object ForToWhile extends PartialTransformer {
    ...
    def myRewriteC: PF[(Command, Env), (Command, Env)] = {
      case (CFor(range, pipeline, par, combine), env) => {
        ...
        if (typ.isDefined == false) {
          throw NotImplemented("Cannot lower `for` loop without iterator type. Add explicit type for the iterator", it.pos)
        }
