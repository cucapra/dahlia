...
object Parser {
  ...
  def and[_: P]: P[BOp] = P("&&".!.map(op => BoolOp(op)))
  ...
  def parse(str: String): Prog = {
    fastparse.parse[Prog](str, prog(_)) match {
      case Parsed.Success(e, _) => e
      case Parsed.Failure(_, _, extra) =>
        throw Errors.ParserError(extra.trace().msg)
    }
  }
}
