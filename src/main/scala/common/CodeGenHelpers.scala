...
object CodeGenHelpers:
  ...
  def log2(n: Int): Double = log10(n) / log10(2)

  def fastDiv(l: Expr, r: Expr): Expr = (l, r) match
  ...
  def fastMod(l: Expr, r: Expr): Expr = (l, r) match
    case (EInt(n, b), EInt(m, _)) => EInt(n % m, b)
  ...
  def and(l: Expr, r: Expr): Expr = (l, r) match
    case (EBool(true), r) => r
    case (l, EBool(true)) => l
  ...
  def binop(op: BOp, l: Expr, r: Expr): Expr = (op, l, r) match
