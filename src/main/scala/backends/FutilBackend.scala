...
private class FutilBackendHelper {
  ...
  def emitDecl(d: Decl): List[Futil.Structure] = d.typ match {
    case TArray(_, dims) => {
      ...
      val mem = Futil.Component(d.id, Futil.Memory(dims.map(_._1)))
      List(
        Futil.NewComp(constComp),
        Futil.Connection(
          Futil.LonePort(constComp),
          Futil.CompPort(mem, "data-in")
        ),
        Futil.NewComp(mem)
      )
    }
    case TBool() | TFloat() | TDouble() => {
      ...
      List(
        Futil
          .Connection(Futil.LonePort(constComp), Futil.CompPort(regComp, "in")),
  ...
  def emitBinop(name: String, prim: Futil.Primitives, e1: Expr, e2: Expr)(
  ...
  def emitExpr(expr: Expr, lhs: Boolean = false)(
    ...
    expr match {
      ...
      case EArrAccess(id, idxs) => {
        ...
        val (gadgets, addrStructs) =
          ports.zipWithIndex.map {
            case (p, i) =>
              val gad = Futil.Component(Futil.genName("gad"), Futil.Ident())
              (
                gad,
                List(
                  Futil.NewComp(gad),
                  Futil.Connection(p, Futil.CompPort(gad, "in")),
                  Futil.Connection(
                    Futil.CompPort(gad, "out"),
                    Futil.CustomPort(s"$id", s"addr$i")
                  )
                )
              )
          }.unzip
        ...
        if (lhs) {
          val writeEn =
            Futil.Component(Futil.genName(s"$id-we"), Futil.Ident())
          ...
          (
            ...
            Futil.Activate(
              List(id, writeEn.id) ++ gadgets.map(_.id) ++ flatActs
            )
          )
        } else {
          (
            Futil.CustomPort(s"$id", "out"),
            structs.flatten ++ addrStructs.flatten,
            Futil.Activate(List(id) ++ gadgets.map(_.id) ++ flatActs)
          )
        }
      }
      ...
      case EApp(Id("sqrt"), List(e)) => {
        ...
        val sqrtStruct =
          List(Futil.NewComp(sqrtComp), Futil.Connection(port, sqrtIn))
        (
          sqrtOut,
          struct ++ sqrtStruct,
          Futil.Activate(List(sqrtComp.id)) ++ act
        )
      }
  ...
  def emitCmd(
      c: Command
  )(implicit store: Store): (List[Futil.Structure], Futil.Control, Store) =
    c match {
      case CPar(c1, c2) => {
      ...
      case CLet(id, _, Some(e)) => {
        ...
        val struct = List(
          Futil.NewComp(reg),
          Futil.Connection(port, Futil.CompPort(reg, "in"))
        ) ++ exStruct
        (struct, Futil.Activate(List(reg.id)) ++ acts, store + (id -> reg.id))
      }
      case CLet(id, _, None) => {
        val reg = Futil.Component(Futil.genName(s"$id"), Futil.Register())
        val struct = List(Futil.NewComp(reg))
      ...
      case CFor(range, _, par, comb) =>
        range match {
          case CRange(id, start, end, 1) => {
            ...
            val iterStart =
              Futil.Constant(Id(s"${iter.id}-start"), Futil.FInt(start))
            val startPort =
              Futil.LonePort(Futil.Component(iterStart.id, iterStart))

            val iterIncr = Futil.Constant(Id(s"${iter.id}-incr"), Futil.FInt(1))
            val incrPort =
              Futil.LonePort(Futil.Component(iterIncr.id, iterIncr))
  ...
  def emitProg(p: Prog, c: Config): String = {
    val _ = c
    val declStruct =
      p.decls.map(x => emitDecl(x)).foldLeft(List[Futil.Structure]())(_ ++ _)
    val store = declStruct.foldLeft(Map[Id, Id]())(
      (store, struct) =>
        struct match {
