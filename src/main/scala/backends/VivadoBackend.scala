...
private class VivadoBackend extends CppLike {
  ...
  def emitType(typ: Type): Doc = typ match {
    ...
    case _:TDouble | _:TStaticDouble => text("double")
    case TSizedDouble(s, 0, un) => text(if (un) s"ap_uint<$s>" else s"ap_int<$s>")
    case TSizedDouble(l, i, un) => text(if (un) s"ap_ufixed<$l,$i>" else s"ap_fixed<$l,$i>")
