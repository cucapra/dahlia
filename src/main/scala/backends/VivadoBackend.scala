...
private class VivadoBackend extends CppLike {
  ...
  def emitType(typ: Type): Doc = typ match {
    ...
    case _:TDouble | _:TRational => text("double")
    case TSizedInt(s, un) => text(if (un) s"ap_uint<$s>" else s"ap_int<$s>")
    case TFixed(t,i,true) => text(if (t==i) s"ap_ufixed<$t>" else s"ap_ufixed<$t,$i>")    
    case TFixed(t,i,false) => text(if (t==i) s"ap_fixed<$t>" else s"ap_fixed<$t,$i>")    
