...
object Cpp {
  ...
  trait CppLike extends PrettyPrinter {
    ...
    implicit def typeToDoc(typ: Type): Doc
        ...
        case CFor(range, par, combine) =>
          "for" <> parens {
            ...
            range.iter <> "++"
              ...
              unroll(range.u) <@>
              par <@>
              (if (combine != CEmpty) text("// combiner:") <@> combine else value(""))
          }
    ...
    def withArrayType(id: Id): Doc = id.typ match {
    ...
    def bankPragmas(decls: List[Decl]) = decls
      ...
      def progToDoc(p: Prog, c: Utils.Config): Doc = {
