...
def stage_hls(db, config):
    ...
    with work(db, 'seashelled', 'hlsing', 'hlsed') as job:
        ...
        c_hw = job['c_main']
        obj_hw = job['obj_main']
        runl(job, prefix + ['sds++', '-sds-pf', platform, '-sds-hw', func_hw, c_hw, '-sds-end', '-clkid', '3', '-poll-mode', '1', '-verbose', '-D', 'SDSOC', '-Wall', '-O3', '-c', '-MMD', '-MP', '-MF"vsadd.d"', c_hw, '-o', obj_hw], cwd=CODE_DIR)
